    ...
    override fun convert2Class(input: Any): ClassStruct?  {
        ...
        input.modelList.forEach{ hiidoModel ->
            ...
            hiidoModel.keyList.forEach {
                val param = Param(it.key, "String", it.value)
                params.add(param)
                if (!it.value.isEmpty()) {
                    // 参数注释
                    methodComments.add("@param ${it.key}, ${it.value}")
                }
                methodContent.append(it.key + ": %s ")
            }
            ...
            method.comment = methodComments
            method.content = methodContent.toString()
            methods.add(method)
        }
